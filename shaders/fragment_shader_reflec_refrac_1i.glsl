...
void calcul_refrac_sphere(int id, Ray ray)
{
	...
	inter_scene[id]=intersection_sphere(rayInterne,id);
		...
		if(id>=0)
		{
			...
			if(objets[id*TAILLE_OBJ+TRANSP]>0. && numRefrac>0)
			{
				...
				if(id>=0 )
				{
					...
					//Réflection du rayon réfracté:
					if(objets[id*TAILLE_OBJ+REFLEC]>0. && numReflec>0)
					{

						calcul_reflection(id,rayRefr);
						...
						couleur_refrac1+=couleur_reflet1*reflec_i1;
					}
				}
			...
			if(objets[id*TAILLE_OBJ+REFLEC]>0. && numReflec>0)
			{
				...
				if(id>=0 )
				{
					...
					if(objets[id*TAILLE_OBJ+TRANSP]>0. && numRefrac>0)
					{
						couleur_refrac1=C_ciel;
						calcul_refraction(id,rayRefl);	//Renvoie le rayon à la sortie de l'objet...
						...
						couleur_reflet1+=couleur_refrac1*reflec_i1;
					}
